...
if __name__ == '__main__':
    ...
    path_input_static = os.path.join(path_input, 'static_datasets')
    path_input_remotes = os.path.join(path_input, 'rs_input_tiffs')
    path_input_model = os.path.join(path_input, 'model_tiff_outputs')
    ...
    basins[ID] = {
        ...
        'lu': os.path.join(path_input_static, 'Cimanuk.tif'),

        'full_basin_mask': os.path.join(path_input_static, 'Cimanuk.tif'),
        'masks': {
            1: ('Full',
                os.path.join(path_input_static, '1_subbasin.tif'),
                [], [])},
        # 'masks': {
        ...
        'crops': [
            (os.path.join(
                path_input_static, 'growing seasons', 'palm_perennial.txt'),
             'Palm Oil', 'Other crops', '-', 52.0),
            (os.path.join(
                path_input_static, 'growing seasons', 'palm_perennial.txt'),
             'Palm Oil', 'Other crops', '-', 33.0),
            (os.path.join(
                path_input_static, 'growing seasons', 'rice_irrigated_java.txt'),
             'Rice - Irrigated', 'Cereals', '-', 54.0),
            (os.path.join(
                path_input_static, 'growing seasons', 'rice_rain_java.txt'),
        ...
        'GRACE': os.path.join(path_input_static, 'Cimanuk_GSFC_average_mmwe.csv'),
    ...
    ###
    global_data = {
        'equiped_sw_irrigation': os.path.join(
            path_input_static,
            r'GMIA_FAO\GMIA_FAO\gmia_v5_aeisw_pct_aei_asc\gmia_v5_aeisw_pct_aei.asc'),
        'wpl_tif': os.path.join(
            path_input_static, r'WPL_Max1.tif'),
        'environ_water_req': os.path.join(
            path_input_static, 'EWR.tif'),
        'population_tif': os.path.join(
            path_input_static, r'IDN_pph_v2b_2010_UNadj_calc_CimanukClip.tif'),
        'cattle': os.path.join(
            path_input_static, r'Glb_Cattle_CC2006_AD_CimanukClip.tif'),
        'dem': os.path.join(
            path_input_static, r'HydroSHED\DEM\DEM_HydroShed_m_3s.tif'),
        'dir': None,
        'root_depth': os.path.join(
            path_input_model, r'c')
    }
    ...
    ###
    data = {
        'ndm_folder': os.path.join(path_input_remotes, 'NDM'),
        'p_folder': os.path.join(path_input_remotes, 'Precipitation'),
        'et_folder': os.path.join(path_input_remotes, 'Evaporation'),
        'n_folder': os.path.join(path_input_remotes, 'Rainy_Days'),
        'lai_folder': os.path.join(path_input_remotes, 'LAI'),
        'etref_folder': os.path.join(path_input_remotes, 'ETref'),

        'bf_folder': os.path.join(path_input_model, 'Baseflow_M'),
        'sr_folder': os.path.join(path_input_model, 'SurfaceRunoff_M'),
        'tr_folder': os.path.join(path_input_model, 'TotalRunoff_M'),
        'perc_folder': os.path.join(path_input_model, 'Percolation_M'),
        'dperc_folder': os.path.join(path_input_model, 'IncrementalPercolation_M'),
        'supply_total_folder': os.path.join(path_input_model, 'Supply_M'),
        'dro_folder': os.path.join(path_input_model, 'IncrementalRunoff_M'),
        'etb_folder': os.path.join(path_input_model, 'ETblue_M'),
        'etg_folder': os.path.join(path_input_model, 'ETgreen_M'),
        'rzsm_folder': os.path.join(path_input_model, 'RootDepthSoilMoisture_M')
    }
