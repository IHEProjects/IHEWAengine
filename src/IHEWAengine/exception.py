...
class IHEClassInitError(Exception):
    ...
    def __init__(self, mod, msg=None):
        if msg is None:
            self.msg = 'Class "{m}" init error.'.format(m=mod)
...
class IHEFileError(Exception):
    ...
    def __init__(self, file, msg=None):
        ...
        if msg is None:
            self.msg = '"{f}" not found in "{p}".'.format(f=fname, p=fpath)
        self.file = fname
        self.path = fpath
...
class IHEKeyError(Exception):
    ...
    def __init__(self, key, val, msg=None):
        if msg is None:
            self.msg = '"{k}" not found in "{v}".'.format(k=key, v=val)
        self.key = key
        self.val = val
...
class IHETypeError(Exception):
    ...
    def __init__(self, vname, rtype, vdata, msg=None):
        if msg is None:
            self.msg = '"{n}" requires {t}, received "{d}".'.format(n=vname,
                                                                    t=rtype,
                                                                    d=type(vdata))
        self.vname = vname
        self.rtype = rtype
...
class IHEStringError(Exception):
    ...
    def __init__(self, vname, msg=None):
        if msg is None:
            self.msg = '"{k}" received empty string.'.format(k=vname)
        self.name = vname
