...
def Nearest_Interpolate(Dir_in, Startdate, Enddate, Dir_out=None):
    ...
    for date in Dates:
        ...
        DOYs_oneMonth = DOYs[np.logical_and((DOYs[:, 1] + 8) >= int(DOY_month_start),
        ...
        for EightDays in DOYs_oneMonth[:, 1]:
            ...
            row = DOYs_oneMonth[np.argwhere(DOYs_oneMonth[:, 1] == EightDays)[0][0], :][
                0]
        ...
        # Define output name
        output_name = os.path.join(Dir_out,
                                   files[int(row)].replace('8-daily', 'monthly'))
