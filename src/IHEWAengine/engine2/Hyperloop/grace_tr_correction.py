...
def calc_var_correction(metadata, complete_data, output_dir,
    ...
    if plot:
        ...
        ax.fill_between(*calc_gwsupply(tss[var_name], a), color='#c48211',
                        label='GW Supply')
        ...
        scalar_array = a[0] * (np.cos((X - a[2]) * (np.pi / 6)) * 0.5 + 0.5) + (
                a[1] * (1 - a[0]))
        ...
        plt.suptitle(
            r"$Supply_{SW} = (\alpha \cdot [\cos(t \cdot \frac{2\pi}{12} - \theta) \cdot \frac{1}{2} + \frac{1}{2}] + \beta \cdot (1 - \alpha)) \cdot Supply_{tot}$" + '\n' + r"$\alpha = $" +
        ...
        if metadata['lu_based_supply_split']:
            plt.plot(grace[0], func(X, 0., 1., 1., slope=False), color='darkblue',
                     label='WAplus (lu)')
            plt.plot(
                *calc_polyfit((grace[0], func(X, 0., 1., 1., slope=False)), order=1),
                color='darkblue', linestyle=':')
            plt.plot(grace[0], func(X, a[0], a[1], a[2], slope=False), color='k',
                     label='WAplus (lu + grace)')
            plt.plot(*calc_polyfit((grace[0], func(X, a[0], a[1], a[2], slope=False)),
                                   order=1), color='k', linestyle=':')
        else:
            plt.plot(grace[0], func(X, a[0], a[1], a[2], slope=False), color='k',
                     label='WAplus (grace)')
            plt.plot(*calc_polyfit((grace[0], func(X, a[0], a[1], a[2], slope=False)),
...
def calc_gwsupply(total_supply, params):
    ...
    scalar_array = params[0] * (np.cos((x - params[2]) * (np.pi / 6)) * 0.5 + 0.5) + (
            params[1] * (1 - params[0]))
...
def correct_var(metadata, complete_data, output_dir, formula,
    ...
    for date, fn in zip(complete_data[var][1], complete_data[var][0]):
        ...
        fraction = a[0] * (np.cos((x - a[2]) * (np.pi / 6)) * 0.5 + 0.5) + (
                a[1] * (1 - a[0]))
