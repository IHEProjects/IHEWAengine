...
def calc_mean(maps):
    ...
    geoinfo = becgis.GetGeoInfo(maps[0])

    data_sum = np.zeros((geoinfo[3], geoinfo[2]))
    data_count = np.zeros((geoinfo[3], geoinfo[2]))

    for filename in maps:
        data = becgis.OpenAsArray(filename, nan_values=True)
    ...
    becgis.CreateGeoTiff(fh, mean, *geoinfo)
...
def calc_sb_indicators(basins, output_dir, pop_map):
    ...
    for basin in list(basins.values()):
        ...
        sb_masks = becgis.ListFilesInFolder(basin['masks'])
...
def calc_avg_flux(mask, basin, output_dir, flux):
    ...
    print(input_dir)
    ets = becgis.SortFiles(input_dir, [-10, -6], month_position=[-6, -4])[0]
...
def calc_monthly_p(basin, output_dir):
    ...
    precip = becgis.SortFiles(input_dir, [-10, -6], month_position=[-6, -4])
...
def calc_mskd_mean(mask_map, pop_map, idc_name):
    ...
    pop_map = becgis.MatchProjResNDV(mask_map, target_maps, temp_dir)[0]

    lu = becgis.OpenAsArray(mask_map, nan_values=True)
    ppl_ha = becgis.OpenAsArray(pop_map, nan_values=True)
