...
def create_sheet7(complete_data, metadata, output_dir, global_data, data):
    ...
    fraction_fhs = split_yield(output_folder, p_fhs, et_blue_fhs, et_green_fhs, ab=(1.0, 1.0))

    # calculate feed production and return filehandles of saved tif files
    feed_fhs_landscape, feed_fhs_incremental = livestock_feed(output_folder,
                                                              lu_fh,
                                                              AREA,
                                                              ndm_fhs,
                                                              feed_dict, live_feed, cattle_fh, fraction_fhs, date_list2)

    # calculate fuel production and return filehandles of saved tif files
    fuel_fhs_landscape, fuel_fhs_incremental = fuel_wood(output_folder,
                                                         lu_fh,
                                                         AREA,
                                                         ndm_fhs,
    ...
    atm_recy_landscape_fhs = recycle(output_folder, et_green_fhs, recy_ratio, lu_fh, 'landscape')
    atm_recy_incremental_fhs = recycle(output_folder, et_blue_fhs, recy_ratio, lu_fh, 'incremental')
...
def dry_season_bf(output_folder, WPixOutFile, dry_months):
    ...
    geo_out = (np.min(lon) - px_size_lon / 2,
               px_size_lon,
               0,
               np.max(lat) - px_size_lat / 2,
               0,
               px_size_lat)
