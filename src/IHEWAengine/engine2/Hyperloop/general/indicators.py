...
def sheet3_indicators(dir1):
    ...
    for (fa, fb) in files:
        ...
        crop_TYPE = ['Cereals', 'Beverage crops', 'Feed crops', 'Fruit & vegetables',
                     'Non-cereals', 'Oilseeds', 'Other crops']
        production_1 = []
        area_1 = []
        ...
        for crop in crop_TYPE:
            ...
            for cl in ['IRRIGATED', 'RAINFED']:
                ...
                if cl == 'IRRIGATED':
                    df_cropclass = df_crop.loc[
                        (df_crop.CLASS == cl) & (df_crop.SUBCLASS == 'Total yield')]
                else:
                    df_cropclass = df_crop.loc[
                        (df_crop.CLASS == cl) & (df_crop.SUBCLASS == 'Yield')]
