...
def create_sheet3(complete_data, metadata, output_dir):
    ...
    for crop in metadata['crops']:
        if crop[4] in LULC:
            ...
            result_seasonly = calc_Y_WP_seasons(start_dates, end_dates, metadata['lu'],
                                                crop[4], crop[1],
                                                complete_data['etg'][0],
                                                complete_data['etg'][1],
                                                complete_data['etb'][0],
            ...
            result = calc_Y_WP_year(result_seasonly,
                                    os.path.join(output_dir, 'WP_Y_Yearly_csvs'),
                                    crop[1])
            plot_Y_WP(result, os.path.join(output_dir, 'WP_Y_Yearly_graphs'),
                      croptype=crop[1], catchment_name=metadata['name'], filetype='png')
            plot_Y_WP(result_seasonly, os.path.join(output_dir, 'WP_Y_Seasonly_graphs'),
                      croptype=crop[1], catchment_name=metadata['name'], filetype='png')
            if crop[4] > 50:
    ...
    if metadata['non_crop'] is not None:
        for i, non_crop in enumerate(
                [metadata['non_crop']['meat'], metadata['non_crop']['milk'],
                 metadata['non_crop']['aquaculture'], metadata['non_crop']['timber']]):
            years[i] = [date.year for date in read_csv(non_crop)[0]][1:]
            crp = ['Meat', 'Milk', 'Aquaculture', 'Timber']
            plot_Y_WP(result, os.path.join(output_dir, 'WP_Y_Yearly_graphs'),
                      croptype=crp[i], catchment_name=metadata['name'], filetype='png')
    ...
    if metadata['non_crop'] is not None:
        ...
        wp_y_non_crop_dictionary['Fish (Aquaculture)']['-'] = metadata['non_crop'][
            'aquaculture']
    ...
    for year in years:
        csv_fh_a, csv_fh_b = create_sheet3_csv(wp_y_irrigated_dictionary,
                                               ...
                                               wp_y_non_crop_dictionary, year,
                                               output_dir)
        output_fh_a = csv_fh_a[:-3] + 'pdf'
        output_fh_b = csv_fh_b[:-3] + 'pdf'
        sheet3a_fh, sheet3b_fh = create_sheet3_png(metadata['name'], str(year),
                                                   ['km3/year', 'kg/ha/year', 'kg/m3'],
                                                   [csv_fh_a, csv_fh_b],
                                                   [output_fh_a, output_fh_b],
                                                   template=[get_path('sheet3_1_svg'),
                                                             get_path('sheet3_2_svg')])

    return complete_data


def create_sheet3_csv(wp_y_irrigated_dictionary, wp_y_rainfed_dictionary,
                      wp_y_non_crop_dictionary, year, output_dir):
    """
    ...
    first_row_b = ["USE", "CLASS", "SUBCLASS", "TYPE", "SUBTYPE", "LAND_PRODUCTIVITY",
                   "WATER_PRODUCTIVITY"]
    ...
    for TYPE in list(wp_y_irrigated_dictionary.keys()):
        for SUBTYPE in list(wp_y_irrigated_dictionary[TYPE].keys()):
            if type(wp_y_irrigated_dictionary[TYPE][SUBTYPE]) is type(None):
                writer_b.writerow(
                    ["CROP", "IRRIGATED", "Yield rainfall", TYPE, SUBTYPE, "nan",
                     "nan"])
                writer_b.writerow(
                    ["CROP", "IRRIGATED", "Incremental yield", TYPE, SUBTYPE, "nan",
                     "nan"])
                writer_b.writerow(
                    ["CROP", "IRRIGATED", "Total yield", TYPE, SUBTYPE, "nan", "nan"])
                writer_a.writerow(
                    ["CROP", "IRRIGATED", "ET rainfall", TYPE, SUBTYPE, "nan"])
                writer_a.writerow(
                    ["CROP", "IRRIGATED", "Incremental ET", TYPE, SUBTYPE, "nan"])
            else:
                start_dates, end_dates, Y, Yirr, Ypr, WP, WPblue, WPgreen, WC, WCblue, WCgreen = read_csv(
                    wp_y_irrigated_dictionary[TYPE][SUBTYPE])
                mask = start_dates == datetime.date(year, 1, 1)
                writer_b.writerow(
                    ["CROP", "IRRIGATED", "Yield rainfall", TYPE, SUBTYPE, Ypr[mask][0],
                     WPgreen[mask][0]])
                writer_b.writerow(
                    ["CROP", "IRRIGATED", "Incremental yield", TYPE, SUBTYPE,
                     Yirr[mask][0], WPblue[mask][0]])
                writer_b.writerow(
                    ["CROP", "IRRIGATED", "Total yield", TYPE, SUBTYPE, Y[mask][0],
                     WP[mask][0]])
                writer_a.writerow(["CROP", "IRRIGATED", "ET rainfall", TYPE, SUBTYPE,
                                   WCgreen[mask][0]])
                writer_a.writerow(["CROP", "IRRIGATED", "Incremental ET", TYPE, SUBTYPE,
                                   WCblue[mask][0]])
    ...
    for TYPE in list(wp_y_rainfed_dictionary.keys()):
        for SUBTYPE in list(wp_y_rainfed_dictionary[TYPE].keys()):
            if type(wp_y_rainfed_dictionary[TYPE][SUBTYPE]) is type(None):
                writer_b.writerow(
                    ["CROP", "RAINFED", "Yield", TYPE, SUBTYPE, "nan", "nan"])
                writer_a.writerow(["CROP", "RAINFED", "ET", TYPE, SUBTYPE, "nan"])
            else:
                start_dates, end_dates, Y, Yirr, Ypr, WP, WPblue, WPgreen, WC, WCblue, WCgreen = read_csv(
                    wp_y_rainfed_dictionary[TYPE][SUBTYPE])
                mask = start_dates == datetime.date(year, 1, 1)
                writer_b.writerow(
                    ["CROP", "RAINFED", "Yield", TYPE, SUBTYPE, Y[mask][0],
                     WP[mask][0]])
    ...
    for TYPE in list(wp_y_non_crop_dictionary.keys()):
        for SUBTYPE in list(wp_y_non_crop_dictionary[TYPE].keys()):
            if type(wp_y_non_crop_dictionary[TYPE][SUBTYPE]) is type(None):
                writer_b.writerow(
                    ["NON-CROP", "RAINFED", "Yield", TYPE, SUBTYPE, "nan", "nan"])
                writer_b.writerow(
                    ["NON-CROP", "IRRIGATED", "Yield rainfall", TYPE, SUBTYPE, "nan",
                     "nan"])
                writer_b.writerow(
                    ["NON-CROP", "IRRIGATED", "Incremental yield", TYPE, SUBTYPE, "nan",
                     "nan"])
                writer_b.writerow(
                    ["NON-CROP", "IRRIGATED", "Total yield", TYPE, SUBTYPE, "nan",
                     "nan"])
                if TYPE != 'Livestock':
                    writer_a.writerow(
                        ["NON-CROP", "RAINFED", "ET", TYPE, SUBTYPE, "nan"])
                    writer_a.writerow(
                        ["NON-CROP", "IRRIGATED", "ET rainfall", TYPE, SUBTYPE, "nan"])
                    writer_a.writerow(
                        ["NON-CROP", "IRRIGATED", "Incremental ET", TYPE, SUBTYPE,
                         "nan"])
                else:
                    continue
            else:
                start_dates, end_dates, Y, Yirr, Ypr, WP, WPblue, WPgreen, WC, WCblue, WCgreen = read_csv(
                    wp_y_non_crop_dictionary[TYPE][SUBTYPE])
                mask = start_dates == datetime.date(year, 1, 1)
                if TYPE != 'Livestock':
                    writer_b.writerow(
                        ["NON-CROP", "RAINFED", "Yield", TYPE, SUBTYPE, Y[mask][0],
                         WP[mask][0]])
                    writer_b.writerow(
                        ["NON-CROP", "IRRIGATED", "Yield rainfall", TYPE, SUBTYPE,
                         "nan", "nan"])
                    writer_b.writerow(
                        ["NON-CROP", "IRRIGATED", "Incremental yield", TYPE, SUBTYPE,
                         "nan", "nan"])
                    writer_b.writerow(
                        ["NON-CROP", "IRRIGATED", "Total yield", TYPE, SUBTYPE, "nan",
                         "nan"])
                    writer_a.writerow(
                        ["NON-CROP", "RAINFED", "ET", TYPE, SUBTYPE, WC[mask][0]])
                    writer_a.writerow(
                        ["NON-CROP", "IRRIGATED", "ET rainfall", TYPE, SUBTYPE, "nan"])
                    writer_a.writerow(
                        ["NON-CROP", "IRRIGATED", "Incremental ET", TYPE, SUBTYPE,
...
def calc_Y_WP_year(csv_fh, output_dir, croptype):
    ...
    start_dates, end_dates, Y, Yirr, Ypr, WP, WPblue, WPgreen, WC, WC_blue, WC_green = read_csv(
        csv_fh)

    years = np.unique(
        np.array([date.year for date in np.append(start_dates, end_dates)]))

    csv_filename = os.path.join(output_dir, r'Yearly_' + os.path.basename(csv_fh))
    csv_file = open(csv_filename, 'w')
    writer = csv.writer(csv_file, delimiter=';', lineterminator='\n')
    writer.writerow(["Startdate", "Enddate", "Yield [kg/ha]", "Yield_pr [kg/ha]",
                     "Yield_irr [kg/ha]", "WP [kg/m3]", "WP_blue [kg/m3]",
                     "WP_green [kg/m3]", "WC [km3]", "WC_blue [km3]", "WC_green [km3]"])

    for year in years:
        starts, ends = (np.array(
            [start_date for start_date, end_date in zip(start_dates, end_dates) if
             start_date.year == year or end_date.year == year]),
                        np.array([end_date for start_date, end_date in
                                  zip(start_dates, end_dates) if
                                  start_date.year == year or end_date.year == year]))
        ...
        lengths_total_season = [float(abs((end - start).days)) for start, end in
                                zip(starts, ends)]

        lengths_within_year = np.array([min(year_length,
                                            abs((boundary - end).days)) - abs(
            min(0, (boundary - start).days)) for start, end in zip(starts, ends)])

        fractions = lengths_within_year / lengths_total_season

        y = np.sum(
            np.array([Y[start_dates == start][0] for start in starts]) * fractions)
        yirr = np.sum(
            np.array([Yirr[start_dates == start][0] for start in starts]) * fractions)
        ypr = np.sum(
            np.array([Ypr[start_dates == start][0] for start in starts]) * fractions)

        wc = np.sum(
            np.array([WC[start_dates == start][0] for start in starts]) * fractions)
        wcblue = np.sum(np.array(
            [WC_blue[start_dates == start][0] for start in starts]) * fractions)
        wcgreen = np.sum(np.array(
            [WC_green[start_dates == start][0] for start in starts]) * fractions)

        wp = np.average(np.array([WP[start_dates == start][0] for start in starts]),
                        weights=fractions)
        wpblue = np.average(
            np.array([WPblue[start_dates == start][0] for start in starts]),
            weights=fractions)
        wpgreen = np.average(
            np.array([WPgreen[start_dates == start][0] for start in starts]),
            weights=fractions)

        writer.writerow(
            [datetime.date(year, 1, 1), datetime.date(year, 12, 31), y, ypr, yirr, wp,
             wpblue, wpgreen, wc, wcblue, wcgreen])
    ...
    with open(csv_fh) as csvfile:
        ...
        for row in reader:
            if np.all([row[0] != 'Start', row[1] != 'End']):
                start_dates = np.append(start_dates, datetime.datetime.strptime(row[0],
                                                                                '%d/%m/%Y').date())
                end_dates = np.append(end_dates, datetime.datetime.strptime(row[1],
                                                                            '%d/%m/%Y').date())
...
def calc_Y_WP_seasons(start_dates, end_dates, lu_fh, lu_class, croptype, etgreen_fhs,
                      etgreen_dates, etblue_fhs, etblue_dates, ndm_fhs, ndm_dates,
                      p_fhs, p_dates, output_dir, HIWC_dict, ab=(1.0, 1.0)):
    """
    ...
    csv_filename = os.path.join(output_dir, 'Yields_WPs_{0}_{1}.csv'.format(croptype,
                                                                            int(
                                                                                lu_class)))
    csv_file = open(csv_filename, 'w')
    writer = csv.writer(csv_file, delimiter=';', lineterminator='\n')
    ...
    for startdate, enddate in zip(start_dates, end_dates):
        Yield, Yield_pr, Yield_irr, Wp, Wp_blue, Wp_green, Wc, Wc_blue, Wc_green = calc_Y_WP_season(
            startdate, enddate, lu_fh, lu_class, croptype, etgreen_fhs, etgreen_dates,
            etblue_fhs, etblue_dates, ndm_fhs, ndm_dates, p_fhs, p_dates, HIWC_dict,
            ab=ab, output_dir=output_dir)

        writer.writerow(
            [startdate, enddate, Yield, Yield_pr, Yield_irr, Wp, Wp_blue, Wp_green, Wc,
             Wc_blue, Wc_green])
...
def calc_Y_WP_season(startdate, enddate, lu_fh, lu_class, croptype, etgreen_fhs,
                     etgreen_dates, etblue_fhs, etblue_dates, ndm_fhs, ndm_dates, p_fhs,
                     p_dates, HIWC_dict, ab=(1.0, 1.0), output_dir=None):
    """
    ...
    common_dates = becgis.common_dates(
        [etblue_dates, etgreen_dates, p_dates, ndm_dates])
    ...
    if season_complete:
        ...
        NDMs = np.stack([becgis.open_as_array(ndm_fhs[ndm_dates == date][0],
                                              nan_values=True) * fraction for
                         date, fraction in zip(req_dates, fractions)], axis=2)
        NDM = np.nansum(NDMs, axis=2)
        del NDMs

        ETGREENs = np.stack([becgis.open_as_array(etgreen_fhs[etgreen_dates == date][0],
                                                  nan_values=True) * fraction for
                             date, fraction in zip(req_dates, fractions)], axis=2)
        ETGREEN = np.nansum(ETGREENs, axis=2)
        del ETGREENs

        ETBLUEs = np.stack([becgis.open_as_array(etblue_fhs[etblue_dates == date][0],
                                                 nan_values=True) * fraction for
                            date, fraction in zip(req_dates, fractions)], axis=2)
        ...
        Ps = np.stack(
            [becgis.open_as_array(p_fhs[p_dates == date][0], nan_values=True) * fraction
             for date, fraction in zip(req_dates, fractions)], axis=2)
        ...
        if output_dir:
            ...
            cmap = LinearSegmentedColormap.from_list('mycmap',
                                                     ['#6bb8cc', '#a3db76', '#d98d8e'])
            plt.contourf(XX, YY, Z, np.arange(0.0, 1.1, 0.1), cmap=cmap)
            plt.colorbar(ticks=np.arange(0.0, 1.1, 0.1),
                         label='Yirr as fraction of total Y [-]', boundaries=[0, 1])
            plt.xlabel('Normalized Precipitation [-]')
            plt.ylabel('ETblue/ET [-]')
            plt.title('Split Yield into Yirr and Ypr')
            plt.suptitle(
                'Z(X,Y) = -(((Y-1) * a)^2 - ((X-1) * b)^2) + 0.5 with a = {0:.2f} and b = {1:.2f}'.format(
                    ab[0], ab[1]))
            plt.scatter(pfraction, etbfraction, color='w', label=croptype,
                        edgecolors='k')
            plt.legend()
            plt.xlim((0, 1))
            plt.ylim((0, 1))
            plt.savefig(os.path.join(output_dir,
                                     '{0}_{1}_{2}_cloud.png'.format(croptype,
                                                                    req_dates[0],
                                                                    req_dates[-1])))
...
def plot_Y_WP(csv_fh, output_dir, croptype=None, catchment_name=None, filetype='png'):
    ...
    ax.bar(start_dates, Yirr, ordinal_enddates - ordinal_startdates, color='#6bb8cc',
           label='Yield from irrigation', linewidth=2, edgecolor='w', align='edge')
    ax.bar(start_dates, Ypr, ordinal_enddates - ordinal_startdates, bottom=Yirr,
           color='#a3db76', label='Yield from precipitation', linewidth=2,
    ...
    ax.legend(handles=[red_patch, blue_line, green_line], loc='upper left',
              fancybox=True, shadow=True)
    ax.bar(ordinal_meandates, WPblue, color='w', linewidth=2, edgecolor='w',
           xerr=(ordinal_enddates - ordinal_startdates) / 2.2, ecolor='#6bb8cc',
           capsize=0, error_kw={'lw': 3})
    ax.bar(ordinal_meandates, WPgreen, color='w', linewidth=2, edgecolor='w',
           xerr=(ordinal_enddates - ordinal_startdates) / 2.2, ecolor='#a3db76',
           capsize=0, error_kw={'lw': 3})
    ax.bar(start_dates, WP, ordinal_enddates - ordinal_startdates, color='#d98d8e',
           label='WP', linewidth=2, edgecolor='w', align='edge')
    ax.set_title(
        'Seasonal Water Productivity, {0} in {1}'.format(croptype, catchment_name))
...
def create_sheet3_png(basin, period, units, data, output, template=False):
    ...
    list(xml_txt_box)[0].text = 'Part 1: Agricultural water consumption (' + units[
        0] + ')'
    ...
    list(xml_txt_box)[0].text = 'Part 2: Land productivity (' + units[
        1] + ') and water productivity (' + units[2] + ')'
