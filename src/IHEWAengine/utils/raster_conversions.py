    ...
    if len(Array_out_shape) == 3:
        ...
        for i in range(0, Array_out_shape[0]):
            ...
            if sys.version_info[0] == 2:
                Array_out_slice = scipy.misc.imresize(
                    np.float_(Array_in_slice),
                    size,
                    interp=interpolation_method, mode='F')
            if sys.version_info[0] == 3:
                Array_out_slice = skimage.transform.resize(
                    np.float_(Array_in_slice),
                    size,
        ...
        if sys.version_info[0] == 2:
            Array_out = scipy.misc.imresize(
                np.float_(Array_in),
                size,
                interp=interpolation_method, mode='F')
        if sys.version_info[0] == 3:
            Array_out = skimage.transform.resize(
                np.float_(Array_in),
                size,
                order=interpolation_number)
